...
namespace Day6
{
    public class UnitTest1
    {
        [Theory]
        [InlineData("abcx\nabcy\nabcz")]
        public void Test1(string answers)
        {
            var sut = new Solver(answers);
...
b";
            ...
            Assert.Equal(11, answers.Replace("\r\n\r\n", "|").Split("|").Sum(p => new Solver(p).Yes));
        }
...
";
            ...
            Assert.Equal(6335, data.Replace("\r\n\r\n", "|").Split("|").Sum(p => new Solver(p).Yes));
        }
...
a", 1)]
        [InlineData(@"b", 1)]
        public void Test3(string answers, int expectedAnswer)
        {
            var sut = new SecondSolver(answers);
...
ljqk";
            ...
            Assert.Equal(3392, data.Replace("\r\n\r\n", "|").Split("|").Sum(p => new SecondSolver(p).Yes));
        }
    }

    public class Solver
    {
        public Solver(string answers)
        {
            Yes = answers.Replace("\r", "").Replace("\n", "").Distinct().Count();
        }
    ...
    public class SecondSolver
    {
        ...
        public SecondSolver(string answers)
        {
            var initialized = false;
            _answers = answers;
            var splitAnswers = answers.Replace("\r\n", "|").Split("|");
            List<char> splitStrings = new List<char>();
            foreach (var splitAnswer in splitAnswers)
            {
                if (! initialized)
                {
                    splitStrings.AddRange(splitAnswer.ToCharArray().OrderBy(p => p));
                    initialized = true;
                }
                else
                {
                    var otherList = new List<char>();
                    otherList.AddRange(splitAnswer.ToCharArray().OrderBy(p => p));
                    splitStrings = splitStrings.Intersect(otherList).ToList();
                }
            }

            Yes = splitStrings.Count;
        }
