...
/*            map.Reassemble();
            Assert.Collection(map.GetReassembledMap(),
                p1 =>
                {
                    Assert.Equal(2311, p1.Id);
                    Assert.Equal(Tile.CurrentPosition.FlippedVertically, p1.Position);
                },
                p2 =>
                {
                    Assert.Equal(1951, p2.Id);
                    Assert.Equal(Tile.CurrentPosition.FlippedVertically, p2.Position);
                });*/
    }
    ...
    public class Map
    {
        ...
        public IEnumerable<Tile> GetReassembledMap()
        {
        ...
        {
             // Find tiles with 2 sides that can't be found anywhere else
            var compatiblity = new List<int>();
            foreach (var tile in _tiles)
            {
                //foreach (var top in tile.Transformations.Select(t => t.Top).Distinct())

                var emptyBorder = 4 - _tiles.Count(t => t.Id != tile.Id && tile.CouldBeAdjacentOf(t));
                if (Width == 1 || Height == 1)
                {
                    if (emptyBorder == 3)
                    {
